...
**Supporting**:
- [x] - `p1` - Imports, constants (GitHub owner/repo, API base, user agent, cache copy allowlist, architecture distributable allowlist), API URL resolver, latest version resolver, local copy function, architecture pruning helper, archive extraction helpers (tar prefix, tar extract, zip prefix, zip extract) - `inst-cache-helpers`
