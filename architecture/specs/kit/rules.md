...
**Location**: `config/kits/<slug>/artifacts/<KIND>/rules.md`
...
3. Resolve kit slug from `config/core.toml` (e.g., `"sdlc"`)
4. Build full path: `config/kits/{slug}/artifacts/{KIND}/rules.md`
...
Format: `` `path` — description ``. All paths are relative to the same kit config directory (`config/kits/{slug}/`).

**taxonomy.md** is an optional dependency — generated by `cypilot generate-resources` at `config/kits/{slug}/taxonomy.md`. It provides artifact kind definitions, ID type descriptions, and cross-artifact traceability context. Workflows MAY use it to understand the kit's naming conventions and artifact relationships.
...
For a PRD artifact, `config/kits/sdlc/artifacts/PRD/rules.md`:
...
|---|-------|----------|---------------|
| S.1 | rules.md exists at correct path | YES | File at `config/kits/<slug>/artifacts/<KIND>/rules.md` |
