...
- [ ] ADR has unique ID: `fdd-{system}-adr-{slug}`
...
- [ ] Superseding ADR: `fdd-{system}-adr-{slug}-v2` with status SUPERSEDED on original
...
4. **For SUPERSEDED**: Add `superseded_by: fdd-{system}-adr-{new-slug}`
...
1. **ADR Checkbox** (`id:adr`):
   - `[ ] p1 - fdd-{system}-adr-{slug}` — unchecked until decision incorporated into design
   - `[x] p1 - fdd-{system}-adr-{slug}` — checked when DESIGN references this ADR and components are implemented
...
**Validation Checks**:
- `fdd validate` will warn if `id:adr` has no references in DESIGN
- `fdd validate` will warn if ADR is ACCEPTED but no `id-ref:adr` exists in DESIGN
- `fdd validate` will warn if `id-ref:adr` is `[x]` but `id:adr` is not
...
- [ ] Generate ID: `fdd-{system}-adr-{slug}`
