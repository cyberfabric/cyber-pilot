...
- [ ] Format: `fdd-{system}-type-{slug}-v2`, `fdd-{system}-comp-{slug}-v3`, etc.
...
1. **Principle Checkbox** (`id:principle`):
   - `[ ] p1 - fdd-{system}-principle-{slug}` — unchecked until principle is applied
   - `[x] p1 - fdd-{system}-principle-{slug}` — checked when ALL features applying this principle are `[x]`

2. **Constraint Checkbox** (`id:constraint`):
   - `[ ] p1 - fdd-{system}-constraint-{slug}` — unchecked until constraint is satisfied
   - `[x] p1 - fdd-{system}-constraint-{slug}` — checked when ALL features satisfying this constraint are `[x]`

3. **Component Checkbox** (`id:component`):
   - `[ ] p1 - fdd-{system}-component-{slug}` — unchecked until component is implemented
   - `[x] p1 - fdd-{system}-component-{slug}` — checked when ALL features using this component are `[x]`

4. **Sequence Checkbox** (`id:seq`):
   - `[ ] p1 - fdd-{system}-seq-{slug}` — unchecked until sequence is implemented
   - `[x] p1 - fdd-{system}-seq-{slug}` — checked when ALL features implementing this sequence are `[x]`

5. **Database Table Checkbox** (`id:dbtable`):
   - `[ ] p1 - fdd-{system}-dbtable-{slug}` — unchecked until table is created
   - `[x] p1 - fdd-{system}-dbtable-{slug}` — checked when ALL features using this table are `[x]`
...
**Validation Checks**:
- `fdd validate` will warn if `id:*` has no references in `covered_by` artifacts
- `fdd validate` will warn if reference is `[x]` but definition is not
...
→ Option 1: Run /fdd-generate PRD first (recommended)
...
→ If PRD needs updates: /fdd-generate PRD UPDATE
...
3. **Document resumption point**:
   ...
   - Resume: Continue with component fdd-{system}-comp-{slug}
   ```
4. **On resume**:
...
- [ ] Generate type IDs: `fdd-{system}-type-{slug}`
